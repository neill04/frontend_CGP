...
export default function DelegadosList() {
  const { delegados, fetchDelegados, loading } = useAdmin();
  ...
  // Filtrar delegados
  const filteredDelegados = delegados?.filter((del) => {
    // Filtro por búsqueda (nombre, apellido, DNI o academia)
    const searchLower = searchTerm.toLowerCase();
    const matchesSearch = 
      del.nombres?.toLowerCase().includes(searchLower) ||
  ...
  if (loading) {
    return (
      <>
        <PageMeta title="Cargando..." description="Cargando delegados" />
  ...
  return (
    <>
      <PageMeta 
        title="Todos los Delegados | Copa Gol Perú" 
        description="Lista completa de delegados registrados"
      />
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold">Todos los Delegados</h2>
            <p className="text-gray-600 text-sm mt-1">
              {filteredDelegados?.length || 0} de {delegados?.length || 0} delegado{delegados?.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        ...
        {filteredDelegados && filteredDelegados.length === 0 && delegados && delegados.length > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center mb-6">
            ...
            <p className="text-yellow-700 mb-4">
              No hay delegados que coincidan con los filtros seleccionados
            </p>
            <button
              onClick={handleClearFilters}
        ...
        {/* Grid de delegados */}
        {filteredDelegados && filteredDelegados.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDelegados.map((del) => (
              <div
                key={del.id}
                ...
                <div className="mt-3 flex-shrink-0 h-5">
                  {del.fechaRegistro && (
                    <p className="text-xs text-gray-400 text-center">
                      Registrado: {formatDate(del.fechaRegistro)}
                    </p>
                  )}
                </div>
          ...
          !loading && delegados?.length === 0 && (
            ...
            <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center">
              ...
              <p className="text-gray-600">
                Los delegados se registran desde cada academia
              </p>
            </div>
          )
        )}
