...
export default function FormEntrenador() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.licencia || formData.licencia.trim().length < 3) {
    ...
    if (!selectedFile && !formData.fotoUrl) {
      errors.fotoUrl = "La foto del entrenador es requerida";
    }
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      showToast("Error al registrar el entrenador. Intenta nuevamente.", "error");
    }
  };

  const handleCancel = () => {
    navigate(`/academias/${id}/entrenadores`);
  };
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
            ...
            <p className="text-gray-600 mb-6">¿Estás seguro de que deseas registrar a este entrenador?</p>
